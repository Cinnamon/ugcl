...
def pipeline_task_IL_no_inter_edge(args, valid=False):
    ...
    model = get_model(dataset, args).to(device=device) if valid else None
    ...
    for task, task_cls in enumerate(args.task_seq):
        ...
        subgraph = subgraph.to(device=device)
        ...
        prev_model = copy.deepcopy(model).to(device=device)
...
def pipeline_task_IL_inter_edge(args, valid=False):
    ...
    model = get_model(dataset, args).to(device=device)
    life_model = importlib.import_module(f'Baselines.{args.method}_model')
    life_model_ins = life_model.NET(model, task_manager, args) if valid else None
...
def pipeline_class_IL_no_inter_edge_independent(args, valid=False):    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    args.method = 'independent'
    epochs = args.epochs if valid else 0
    ...
    for task, task_cls in enumerate(args.task_seq):
        ...
        # NOTE: we reload the model to make it independent with previous task
        life_model_ins = life_model.NET(model, task_manager, args) if valid else None
    ...
    # NOTE: we recalculate acc mean for independent for only diagonal entries
    acc_mean = []
    for t in range(args.n_tasks):
        acc = round(acc_matrix[t][t], 2)
        acc_mean.append(acc)

    print('AP: ', round(np.mean(acc_mean)), 2)
...
def pipeline_class_IL_no_inter_edge_minibatch_independent(args, valid=False):    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    args.method = 'independent'
    epochs = args.epochs if valid else 0
