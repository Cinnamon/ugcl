...
def evaluate_batch(args,model, g, features, labels, mask, label_offset1, label_offset2, cls_balance=True, ids_per_cls=None):
    ...
    with torch.no_grad():
        ...
        output = torch.tensor([]).to(device=device)
        output_l = torch.tensor([]).to(device=device)
        ...
        if cls_balance:
            return accuracy(logits, labels.to(device=device), cls_balance=cls_balance, ids_per_cls=ids_per_cls)
        else:
            return accuracy(logits[mask], labels[mask].to(device=device), cls_balance=cls_balance, ids_per_cls=ids_per_cls)
