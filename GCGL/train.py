...
if __name__ == '__main__':
    ...
    parser.add_argument('--lwf_args', type=str2dict, default={'lambda_dist': [0.1,1.,10.], 'T': [0.2, 2.0, 20.0]})
    ...
    parser.add_argument('--ergnn_args', type=str2dict, default={'budget': [10, 100], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM']},
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    parser.add_argument('--erreplace_args', type=str2dict, default={'budget': [20, 200], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM'], 'max_size': [0.5]},
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    parser.add_argument('--dce_args', type=str2dict, default={'budget': [20, 200], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM'], 'max_size': [0.5]},
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    parser.add_argument('--sl_args', type=str2dict, default={'budget': [20, 200], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM'], 'max_size': [0.5]},
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    parser.add_argument('--our_args', type=str2dict, default={'budget': [20, 200], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM'], 'max_size': [0.5]},
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    ...
    for hyp_params in hyp_param_list:
        ...
        if os.path.isfile(f"{args['result_path']}/{name}.pkl") and not args['overwrite_result']:
            ...
            AP_AF = show_final_APAF(f"{args['result_path']}/{name}.pkl", GCGL=True) if args['dataset'] in ['Aromaticity-CL', 'ENZYMES-CL'] else show_final_APAF_f1(f"{args['result_path']}/{name}.pkl")
