    ...
    if not os.path.exists(directory):
        try:
            ...
            if e.errno != errno.EEXIST:
                raise
def predict(args, model, bg, task_i=None, device=None):
    ...
    if args.get('edge_featurizer', None) is not None:
        ...
        if args['backbone'] in ['GCN', 'GAT', 'Weave']:
            return model(bg.to(device=device), node_feats, edge_feats)
        else:
            return model(bg.to(device=device), node_feats, edge_feats, task_i)
    else:
        if args['backbone'] in ['GCN', 'GAT', 'Weave']:
            return model(bg.to(device=device), node_feats)
        else:
            return model(bg.to(device=device), node_feats, task_i)



def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer, task_i, device=None):
    model.train()
    train_meter = Meter()
...
def eval_single_task_multi_label(args, model, data_loader, task_i, device=None):
...
def pipeline_multi_label(args, valid=False, device=None):
    '''
    ...
    if args['pre_trained']:
        ...
        loss_criterion = BCEWithLogitsLoss(pos_weight=dataset.task_pos_weights(train_set.indices).to(device=device),
                                           reduction='none')
    ...
    for tid, task_i in enumerate(args['tasks']):
        if args['method'] == 'jointtrain' and life_model_ins is not None:
            ...
            model = load_model(args).to(device=device)
        ...
        if not valid:
            model = pickle.load(open(save_model_path,'rb')).to(device=device)
        ...
        if args['method'] in ['lwf', 'dce', 'our', 'sl']:
            prev_model = copy.deepcopy(life_model_ins).to(device=device) if valid else None
...
def pipeline_multi_class(args, valid=False, device=device):
    epochs = args['num_epochs'] if valid else 0
    ...
    for tid, task_i in enumerate(args['tasks']):
        ...
        if args['method'] in ['lwf', 'dce', 'sl', 'our']:
            prev_model = copy.deepcopy(life_model_ins.net).to(device=device) if valid else None
