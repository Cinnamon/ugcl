...
def BinaryCrossEntropy(logits, labels, T, device=None):
    labels = Variable(labels.data, requires_grad=False).to(device=device)
    return F.binary_cross_entropy_with_logits(logits/T,labels/T)

def MultiClassCrossEntropy(logits, labels, T, device=None):
    labels = Variable(labels.data, requires_grad=False).to(device=device)
...
class NET(torch.nn.Module):
    ...
    def forward(self, args, bg, device=None):
        logits = predict(args, self.net, bg)
        return logits

    def observe(self, data_loader, loss_criterion, task_i, args, prev_model, device=None):
        """
    ...
    def observe_tskIL_multicls(self, data_loader, loss_criterion, task_i, args, prev_model, device=None):
        """
    ...
    def observe_clsIL(self, data_loader, loss_criterion, task_i, args, prev_model, device=None):
        """
